...
class MOEManager:
    ...
    def __init__(self):
        ...
        self._kappa_slope_scale_abs_top5p_mean_buffer = None
    ...
    def reset(self, name):
        ...
        if name == "selected_scores":
            self._selected_scores_size = 0
            return
        if name == "kappa_slope_scale_abs_top5p_mean":
            self._kappa_slope_scale_abs_top5p_mean_size = 0
            return
    ...
    def add(self, name, value):
        ...
        if name == "kappa_slope_scale_abs_top5p_mean":
            with torch.inference_mode(False):
                if self._kappa_slope_scale_abs_top5p_mean_buffer is None:
                    self._kappa_slope_scale_abs_top5p_mean_buffer = torch.empty(
                        (self._tensor_var_capacity,),
                        device=value.device,
                        dtype=value.dtype,
                    )
                new_size = self._kappa_slope_scale_abs_top5p_mean_size + 1
                self._kappa_slope_scale_abs_top5p_mean_buffer[
                    self._kappa_slope_scale_abs_top5p_mean_size:new_size
                ].copy_(value.reshape(1))
                self._kappa_slope_scale_abs_top5p_mean_size = new_size
            return
        if name == "kappa_slope_scale_abs_bottom5p_mean":
            with torch.inference_mode(False):
                if self._kappa_slope_scale_abs_bottom5p_mean_buffer is None:
                    self._kappa_slope_scale_abs_bottom5p_mean_buffer = torch.empty(
                        (self._tensor_var_capacity,),
                        device=value.device,
                        dtype=value.dtype,
                    )
                new_size = self._kappa_slope_scale_abs_bottom5p_mean_size + 1
                self._kappa_slope_scale_abs_bottom5p_mean_buffer[
        ...
        if name == "kappa_slope_scale_abs_mean":
            with torch.inference_mode(False):
                if self._kappa_slope_scale_abs_mean_buffer is None:
                    self._kappa_slope_scale_abs_mean_buffer = torch.empty(
                        (self._tensor_var_capacity,),
                        device=value.device,
                        dtype=value.dtype,
                    )
                new_size = self._kappa_slope_scale_abs_mean_size + 1
                self._kappa_slope_scale_abs_mean_buffer[
    ...
    def aggregate(self, name):
        ...
        if name == "drop_rate_per_ks":
            ...
            return values
        elif name == "kappa_slope_scale_abs_top5p_mean":
            if (
                self._kappa_slope_scale_abs_top5p_mean_buffer is None
                or self._kappa_slope_scale_abs_top5p_mean_size == 0
            ):
                return None
            values = self._kappa_slope_scale_abs_top5p_mean_buffer[
                :self._kappa_slope_scale_abs_top5p_mean_size
            ]
            return values
        elif name == "kappa_slope_scale_abs_bottom5p_mean":
            if (
                self._kappa_slope_scale_abs_bottom5p_mean_buffer is None
                or self._kappa_slope_scale_abs_bottom5p_mean_size == 0
            ):
                return None
