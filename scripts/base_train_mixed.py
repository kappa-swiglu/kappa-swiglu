...
def chat_sft_data_generator_bos_bestfit(dataset, tokenizer, device_batch_size, max_seq_len, device, device_type, ddp_rank, ddp_world_size, buffer_size=100, resume_state_dict=None):
    ...
    if dataset_size <= 0:
        raise ValueError("chat SFT dataset must be non-empty")
    ...
    if resume_state_dict is not None:
        cursor = int(resume_state_dict.get("cursor", cursor))
        consumed = int(resume_state_dict.get("consumed", consumed))
        skipped_overlong = int(resume_state_dict.get("skipped_overlong", skipped_overlong))
        epoch = int(resume_state_dict.get("epoch", epoch))
        iteration = int(resume_state_dict.get("iteration", iteration))
...
while True:
    ...
    if should_terminate_after_checkpoint or is_last_step or (step > 0 and step != args.resume_from_step and args.save_every > 0 and step % args.save_every == 0):
        ...
        save_checkpoint(
            ...
            { # metadata saved as json
                ...
                "chat_sft_dataloader_state_dict": chat_sft_dataloader_state_dict,
    ...
    train_source = "chat_sft" if should_use_chat_sft_step(step, args.chat_sft_every) else "base"
    ...
    if train_source == "chat_sft":
        epoch = chat_sft_dataloader_state_dict["epoch"]
    else:
